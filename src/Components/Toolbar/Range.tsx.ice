...
import { defaultTrainingStringOptions } from '../TypeTrainer';
...
const { minLength: defaultMin, maxLength: defaultMax } = defaultTrainingStringOptions.wordLength
...
class CustomizedRange extends React.Component<{}, {}> {
  ...
  onLowerBoundChange = (e: { target: { value: React.ReactText; }; }): void => {
    this.setState({ lowerBound: +e.target.value });
  };

  onUpperBoundChange = (e: { target: { value: React.ReactText; }; }): void => {
...
class DynamicBounds extends React.Component {
  ...
  onMinChange = (e: { target: { value: React.ReactText; }; }) => {
    this.setState({
      min: +e.target.value || 0,
    });
  };

  onMaxChange = (e: { target: { value: React.ReactText; }; }) => {
    this.setState({
